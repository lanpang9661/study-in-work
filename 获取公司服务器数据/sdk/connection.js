...
function _parseMessageType(msginfo) {
  ...
  if (receiveinfo && receiveinfo.length > 0 && receiveinfo[0].namespaceURI === "urn:xmpp:receipts") {
    ...
    let ackInfo = msginfo.getElementsByTagName("acked");
...
connection.prototype.handleAckedMessage = function (message) {
  var id = message.id;
  var body1 = message.getElementsByTagName('fromId');
  var from = message.getAttribute('from');
  var idx = from.indexOf('_');
  var end = from.indexOf('@easemob.com');
  if (idx != -1 && end != -1) {
    var fromId = from.substring(idx + 1, end) 
    var msg = {
      mid: fromId,
      sssId: "123123"
    };
    ...
    console.log('from pattern is wrong')
  }

  // var body = message.getElementsByTagName('acked');
  // var midString = body[0].toString();
  // var mid = 0;
  // // mid = body[0].innerHTML;
  // midString = midString.substring(midString.indexOf("id="),midString.length);
  // midString = midString.replace('id="',"");
  // midString = midString.replace('"/>', "");
  // console.log("caonima", midString);
